...
async def limiter(redis_client: redis.Redis, key: str, limit: int, window: int):
...
async def locker_get(redis_client: redis.Redis, key: str):
...
async def locker_set(redis_client: redis.Redis, key: str, lock: bool):
