...
vote_admin_user: models_users.CoreUser
...
async def init_objects() -> None:
    global vote_admin_user, AMAP_user

    vote_admin_user = await create_user_with_groups(
        [GroupType.admin_vote, GroupType.admin_amap],
    )
    ...
    voters_AMAP = models_campaign.VoterGroups(
        group_id=GroupType.admin_amap,
    )
    await add_object_to_db(voters_AMAP)
    voters_admin_vote = models_campaign.VoterGroups(
        group_id=GroupType.admin_vote,
    )
    await add_object_to_db(voters_admin_vote)
...
def test_add_sections(client: TestClient) -> None:
    token = create_api_access_token(vote_admin_user)
...
def test_delete_voter_by_group_id(client: TestClient) -> None:
    token = create_api_access_token(vote_admin_user)
    response = client.delete(
        f"/campaign/voters/{GroupType.admin_vote}",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status_code == 204
...
def test_add_list(client: TestClient) -> None:
    ...
    response = client.post(
        ...
        json={
            ...
            "members": [{"user_id": vote_admin_user.id, "role": "Prez"}],
