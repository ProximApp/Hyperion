...
group_user: models_groups.CoreGroup
...
async def init_objects() -> None:
    global group_user
    group_user = await create_groups_with_permissions(
        permissions=[AmapPermissions.manage_amap],
    ...
    user_with_group = await create_user_with_groups([group_user.id])
...
def test_update_batch_create_users(client: TestClient) -> None:
    response = client.post(
        ...
        json=[
            {
                "email": "1@1.fr",
                "default_group_id": group_user.id,
            },
            {"email": "2@1.fr"},
            {"email": "3@b.fr"},
        ],
...
def test_recover_with_non_existing_account(
    ...
    mocked_hyperion_security_logger.assert_called_once_with(
        ...
        extra={"email": "non-existing@myecl.fr"},
    )
