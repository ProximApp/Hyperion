...
async def test_get_participant_for_school_as_admin(
    ...
    response = client.get(
        f"/competition/participants/schools/{SchoolType.base_school.value}",
        headers={"Authorization": f"Bearer {admin_token}"},
    )
    assert response.status_code == 200, response.json()
    users = response.json()
    assert len(users) > 0
    assert all(user["edition_id"] == str(active_edition.id) for user in users)
    assert all(user["school_id"] == str(SchoolType.base_school.value) for user in users)
...
async def test_get_global_podiums(
    ...
    centrale_score = next(
        (p for p in podiums if p["school_id"] == str(SchoolType.base_school.value)),
        None,
    )
...
async def test_post_pompom_podium_as_admin(
    ...
    centrale = next(
        (
            ...
            if p["school_id"] == str(SchoolType.base_school.value)
        ),
        None,
    )
